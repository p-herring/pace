...
import { errorParam as message, isPlanEditable, parsePaceInput, parseRoutePath, routeDistanceKm, safeNextPath, withParam } from "@/lib/muster-action-helpers";
...
export async function paceSignUpAction(formData: FormData) {
  if (isRateLimited(`sign-up:${await clientIp()}`, 5, 10 * 60 * 1000)) {
    redirect(message("/muster/sign-up", "Too many attempts. Try again in a few minutes."));
  }
  ...
  if (!parsed.success) redirect(message("/muster/sign-up", "Check your name, username, email, password and acceptance of the beta rules."));
  if (!hasSupabaseAuth) redirect(message("/muster/sign-up", "The Muster database is not configured yet."));
  ...
  if (hasSupabaseService) {
    ...
    const { data: existingUsername } = await serviceClient!
      ...
      .maybeSingle();
    if (existingUsername) {
      redirect(message("/muster/sign-up", "That username is already taken. Try another one."));
    }
  }
  ...
  const { data: signUpData, error } = await supabase!.auth.signUp({
    ...
    options: {
      emailRedirectTo: `${origin}/auth/callback?next=/muster/onboarding`,
      data: { display_name: parsed.data.displayName, username: parsed.data.username },
    },
  });
  if (error) redirect(message("/muster/sign-up", error.message));
  ...
  redirect(`/muster/check-email?email=${encodeURIComponent(parsed.data.email)}`);
}
...
export async function paceSignInAction(formData: FormData) {
  ...
  if (isRateLimited(`sign-in:${await clientIp()}`, 10, 5 * 60 * 1000)) {
    redirect(message("/muster/sign-in", "Too many attempts. Try again in a few minutes."));
  }
  ...
  if (!hasSupabaseAuth) {
    redirect(message("/muster/sign-in", "The Muster database is not configured yet."));
  }

  if (!identifier || !password) {
    redirect(message("/muster/sign-in", "Enter your email or username and password."));
  }
  ...
  if (!identifier.includes("@")) {
    if (!hasSupabaseService) {
      redirect(message("/muster/sign-in", "Username sign-in is being configured. Please use your email for now."));
    }
    const serviceClient = createServiceSupabaseClient();
  ...
  if (error) redirect(message("/muster/sign-in", error.message));
  redirect(nextPath);
}
...
export async function paceForgotPasswordAction(formData: FormData) {
  ...
  if (isRateLimited(`forgot-password:${await clientIp()}`, 5, 15 * 60 * 1000)) {
    redirect(message("/muster/forgot-password", "Too many attempts. Try again in a few minutes."));
  }

  if (!email) {
    redirect(message("/muster/forgot-password", "Enter the account email first."));
  }

  if (!hasSupabaseAuth) {
    redirect(withParam("/muster/forgot-password", "message", "Password recovery will work once the Muster database is configured."));
  }
  ...
  const { error } = await supabase!.auth.resetPasswordForEmail(email, {
    redirectTo: `${origin}/auth/callback?next=/muster/update-password`,
  });

  if (error) redirect(message("/muster/forgot-password", error.message));
  redirect(
    withParam(
      "/muster/forgot-password",
      "message",
      "If that email has a Muster account, a reset link is on its way.",
    ),
  );
}
...
export async function paceUpdateProfileAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  const supabase = await createServerSupabaseClient();
  if (!supabase) return { error: "The Muster database is not configured yet." };
  ...
  revalidatePath("/muster/account");
  redirect(withParam("/muster/account", "message", "Profile updated."));
}
...
export async function paceDeleteAccountAction(formData: FormData) {
  if (String(formData.get("confirm") ?? "") !== "delete my account") {
    redirect(message("/muster/account/delete", "Type the confirmation phrase exactly to continue."));
  }

  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message("/muster/account/delete", "The Muster database is not configured yet."));
  const { data: { user } } = await supabase!.auth.getUser();
  if (!user) redirect("/muster/sign-in");

  const { error: prepError } = await supabase!.rpc("pace_prepare_account_deletion");
  if (prepError) redirect(message("/muster/account/delete", prepError.message));

  const serviceClient = createServiceSupabaseClient();
  if (!serviceClient) {
    redirect(
      message(
        "/muster/account/delete",
  ...
  if (deleteError) redirect(message("/muster/account/delete", deleteError.message));

  await supabase!.auth.signOut();
  redirect(withParam("/", "message", "Your Muster account and data have been deleted."));
}
...
export async function paceJoinPlanAction(formData: FormData) {
  ...
  const fallback = `/muster/plan/${planId}`;
  const returnTo = safeNextPath(formData.get("redirectTo"), fallback);
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message(returnTo, "The Muster database is not configured yet."));
  ...
  revalidatePath(`/muster/plan/${planId}`);
...
export async function paceMarkPlanCompletedAction(formData: FormData) {
  ...
  revalidatePath("/muster");
  revalidatePath(`/muster/plan/${planId}`);
  if (user) revalidatePath(`/muster/profile/${user.id}`);
...
export async function paceCreateActivityPostAction(formData: FormData) {
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message("/muster/profile", "The Muster database is not configured yet."));
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/muster/sign-in");
  ...
  if (!parsed.success) {
    redirect(message(`/muster/profile/${user.id}`, "Keep your caption under 1000 characters."));
  }
  ...
  if (!caption && photos.length === 0) {
    redirect(message(`/muster/profile/${user.id}`, "Add a caption or at least one photo."));
  }

  for (const photo of photos) {
    if (!ACTIVITY_PHOTO_TYPES.has(photo.type)) {
      redirect(message(`/muster/profile/${user.id}`, "Photos must be JPG, PNG or WebP."));
    }
    if (photo.size > ACTIVITY_PHOTO_MAX_BYTES) {
      redirect(message(`/muster/profile/${user.id}`, "Keep each photo under 5MB."));
    }
  }
  ...
  if (planId) {
    ...
    if (!plan || new Date(plan.starts_at).getTime() > Date.now()) {
      redirect(message(`/muster/profile/${user.id}`, "Choose a past plan you joined."));
    }
  }
  ...
  if (postError || !post) {
    redirect(message(`/muster/profile/${user.id}`, postError?.message ?? "Could not create that activity post."));
  }
  ...
  if (photos.length > 0 && !serviceClient) {
    ...
    redirect(message(`/muster/profile/${user.id}`, "Photo uploads are not configured yet."));
  }
  ...
  try {
    ...
    redirect(
      message(
        `/muster/profile/${user.id}`,
        activityError instanceof Error ? activityError.message : "Could not upload those photos.",
      ),
    );
  }

  revalidatePath(`/muster/profile/${user.id}`);
  redirect(withParam(`/muster/profile/${user.id}`, "message", "Activity posted."));
}
...
export async function paceDeleteActivityPostAction(formData: FormData) {
  ...
  const { error } = await supabase
    ...
    .eq("id", postId)
    .eq("user_id", user.id);
  if (error) redirect(message(`/muster/profile/${user.id}`, error.message));
  ...
  revalidatePath(`/muster/profile/${user.id}`);
  redirect(withParam(`/muster/profile/${user.id}`, "message", "Activity post deleted."));
}
...
export async function paceSendPlanMessageAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  revalidatePath(`/muster/plan/${parsed.data.planId}`);
  return { sentAt: new Date().toISOString() };
}

export async function paceCreateForumPostAction(formData: FormData) {
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message("/muster/forum", "The Muster database is not configured yet."));
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/muster/sign-in");

  if (isRateLimited(`forum-post:${user.id}`, 1, 20 * 1000)) {
    redirect(message("/muster/forum", "Give it a few seconds before posting again."));
  }
  ...
  if (!parsed.success) {
    redirect(message("/muster/forum", "Add a title and question before posting."));
  }
  ...
  if (error || !post) redirect(message("/muster/forum", error?.message ?? "Could not create that post."));
  revalidatePath("/muster/forum");
  redirect(`/muster/forum/${post.id}`);
}

export async function paceCreateForumReplyAction(formData: FormData) {
  const postId = String(formData.get("postId") ?? "");
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message(`/muster/forum/${postId}`, "The Muster database is not configured yet."));
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/muster/sign-in");

  if (isRateLimited(`forum-reply:${user.id}`, 5, 60 * 1000)) {
    redirect(message(`/muster/forum/${postId}`, "Slow down a touch before replying again."));
  }
  ...
  if (!parsed.success) {
    redirect(message(`/muster/forum/${postId}`, "Write a reply before posting."));
  }
  ...
  if (error) redirect(message(`/muster/forum/${parsed.data.postId}`, error.message));
  revalidatePath("/muster/forum");
  revalidatePath(`/muster/forum/${parsed.data.postId}`);
  redirect(withParam(`/muster/forum/${parsed.data.postId}`, "message", "Reply posted."));
}
...
export async function paceDeleteForumPostAction(formData: FormData) {
  ...
  if (error) redirect(message(`/muster/forum/${postId}`, error.message));
  revalidatePath("/muster/forum");
  redirect(withParam("/muster/forum", "message", "Post deleted."));
}
...
export async function paceDeleteForumReplyAction(formData: FormData) {
  ...
  if (error) redirect(message(`/muster/forum/${postId}`, error.message));
  revalidatePath(`/muster/forum/${postId}`);
  redirect(withParam(`/muster/forum/${postId}`, "message", "Reply deleted."));
}
...
export async function paceReportPlanAction(formData: FormData) {
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect(message("/muster", "The Muster database is not configured yet."));
  const { data: { user } } = await supabase!.auth.getUser();
  if (!user) redirect("/muster/sign-in");
  ...
  if (!parsed.success) redirect(message(`/muster/report/${String(formData.get("planId") ?? "")}`, "Choose a reason for the report."));
  ...
  if (error) redirect(message(`/muster/report/${parsed.data.planId}`, error.message));
  redirect(withParam("/muster", "message", "Thanks — a beta admin will review this report."));
}
...
async function uploadGpxFile(formData: FormData, planId: string): Promise<{ path?: string; error?: string }> {
  ...
  const { error } = await serviceClient.storage.from("muster-routes").upload(path, candidate, {
...
export async function paceUpdatePlanAction(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!existing || existing.host_id !== user.id) redirect("/muster");
  if (!isPlanEditable(existing.starts_at)) {
    redirect(message(`/muster/plan/${planId}`, "This plan starts within 48 hours, so it can no longer be edited."));
  }
  ...
  revalidatePath("/muster");
  revalidatePath(`/muster/plan/${planId}`);
  redirect(withParam(`/muster/plan/${planId}`, "message", "Plan updated."));
}
