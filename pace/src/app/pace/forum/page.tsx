...
import { paceCreateForumPostAction } from "@/app/actions/pace";
    ...
    <main className="pace-shell min-h-screen">
      ...
      <section className="forum-page mx-auto max-w-6xl px-5 pb-20 sm:px-8">
        <div className="forum-hero">
          <div>
            <p className="pace-kicker">Community</p>
            <h1>Forum</h1>
        ...
        <div className="forum-layout">
          <form action={paceCreateForumPostAction} className="forum-composer">
            <p className="pace-kicker">New post</p>
            <h2>Ask the group</h2>
            <label>
              Title
          ...
          <section className="forum-list" aria-labelledby="forum-list-title">
            <div className="forum-list-head">
              <div>
                <p className="pace-kicker">Recent</p>
              ...
              <div className="forum-category-filter" aria-label="Forum categories">
                <Link href="/pace/forum" aria-current={!selectedCategory ? "page" : undefined}>
                  All
                </Link>
                {categories.map((item) => (
                  <Link
                    key={item}
                    href={`/pace/forum?category=${encodeURIComponent(item)}`}
                ...
                return (
                  <article className="forum-post-card" key={post.id}>
                    <div className="forum-post-card-main">
                      ...
                      <h3>
                        <Link href={`/pace/forum/${post.id}`}>{post.title}</Link>
                      </h3>
                      <p>{postPreview(post.body)}</p>
                      <div className="forum-post-meta">
                        <Avatar name={author?.display_name ?? "Pace member"} avatarUrl={author?.avatar_url} size={26} />
