...
import { paceJoinPlanAction, paceLeavePlanAction, paceMarkPlanCompletedAction } from "@/app/actions/muster";
...
export default async function PlanDetail({
  ...
  const confirmed = (confirmedParticipants ?? []).map((row) => {
    ...
    return { name: profile?.display_name ?? "Muster member", avatarUrl: profile?.avatar_url ?? null };
  });
        ...
        <article className="plan-detail-card">
          ...
          <div className="plan-detail-body">
            ...
            <div className="plan-detail-host-row">
              <Avatar name={host?.display_name ?? "Muster member"} avatarUrl={host?.avatar_url} size={36} />
              <div>
                <span>
                  Hosted by{" "}
                  <Link href={`/muster/profile/${plan.host_id}`} className="muster-text">
                    {host?.display_name ?? "a Muster member"}
                  </Link>
                </span>
            ...
            {privateLocation ? (
              <div className="plan-detail-location">
                <p className="muster-kicker">Meeting point</p>
            ...
            <div className="plan-detail-actions">
              {isHost ? (
                editable ? (
                  <Link className="muster-primary" href={`/muster/plan/${plan.id}/edit`}>
                    Edit plan
                  </Link>
                ) : null
              ) : invited ? (
                <>
                  <form action={paceJoinPlanAction}>
                    ...
                    <button type="submit" className="muster-primary">
                      Accept invite
                    </button>
                  </form>
                  <form action={paceLeavePlanAction}>
                    <input type="hidden" name="planId" value={plan.id} />
                    <button type="submit" className="muster-secondary">
                      Decline
                    </button>
                  </form>
                </>
            ...
            {!isHost ? (
              <div className="plan-detail-meta-row">
                <Link href={`/muster/report/${plan.id}`} className="muster-text">
                  Report this plan
                </Link>
              </div>
            ) : null}
