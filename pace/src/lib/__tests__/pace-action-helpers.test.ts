...
import {
  ...
  safeNextPath,
  withParam,
} from "@/lib/pace-action-helpers";

describe("withParam / errorParam", () => {
  it("encodes the value into the query string", () => {
    expect(withParam("/pace/sign-in", "error", "bad credentials")).toBe(
      "/pace/sign-in?error=bad%20credentials",
    );
  });

  it("errorParam is withParam with key='error'", () => {
    expect(errorParam("/pace/new", "oops")).toBe("/pace/new?error=oops");
  });

  it("encodes special characters that could otherwise break the URL", () => {
    expect(withParam("/pace", "message", "you’re on the plan & confirmed")).toContain(
...
describe("safeNextPath", () => {
  it("accepts a normal relative path", () => {
    expect(safeNextPath("/pace/new")).toBe("/pace/new");
  });

  it("falls back for a missing value", () => {
    expect(safeNextPath(null)).toBe("/pace");
  });

  it("falls back for a non-string value", () => {
    const file = new File([""], "x.txt");
    expect(safeNextPath(file)).toBe("/pace");
  });

  it("falls back for an absolute URL", () => {
    expect(safeNextPath("https://evil.example.com")).toBe("/pace");
  });

  it("rejects protocol-relative redirect targets (the open-redirect case)", () => {
    expect(safeNextPath("//evil.example.com")).toBe("/pace");
    expect(safeNextPath("///evil.example.com")).toBe("/pace");
  });

  it("respects a custom fallback", () => {
    expect(safeNextPath(null, "/pace/account")).toBe("/pace/account");
  });
});
